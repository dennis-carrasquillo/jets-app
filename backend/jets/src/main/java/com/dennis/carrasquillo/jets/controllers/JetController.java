...
public class JetController {
	...
	@CrossOrigin(origins = "http://localhost:4200")
	@PutMapping("/{id}")
	public ResponseEntity<Jet> updateJet(@PathVariable int id, @RequestBody Jet jetDetails) {
		Jet jet = jetRepository
				.findById(id).orElseThrow(()-> new ResourceNotFoundException("The Jet with this id: " + id + " does not exist!"));
		jet.setJetName(jetDetails.getJetName());
		jet.setSpeed(jetDetails.getSpeed());
		jet.setIntroduced(jetDetails.getIntroduced());
		Jet updateJet = jetRepository.save(jet);
		return ResponseEntity.ok(updateJet);
	}
}
